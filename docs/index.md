# Payment Channels
Btcnano includes classes necessary to handle payment channel smart contracts. The approach taken is networking-agnostic, so this library can be used at any layer.
...
```
npm install btcnano-channel --save
```

For client-side projects:

```
bower install btcnano-channel --save
```
...
```javascript
var Consumer = require('btcnano-channel').Consumer;
...
Now that we have instantiated our object, we have two ways of funding the channel. The basic one is to send btcnanos to an address that is provided by the Consumer instance (a private key is created for this purpose).

```javascript
console.info('Send btcnanos to ' + consumer.fundingAddress.toString() ' to fund the channel');

consumer.processFunding([{...}, {...}, {...}]);
```

The objects that the consumer can understand are those returned by the Insight API: [https://github.com/bitcoinnano/insight-api-btcnano#unspent-outputs](https://github.com/bitcoinnano/insight-api-btcnano#unspent-outputs)
...
```javascript
var Provider = require('btcnano-channel').Provider;
